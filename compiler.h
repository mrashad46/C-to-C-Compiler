...
enum{
    ...
    TOKEN_TYPE_OPERATOR,     // Operators are used in expressions such as 5+20 "+" would be the operator
    TOKEN_TYPE_SYMBOL,       // Quotes, semicolons, colons and more
    TOKEN_TYPE_NUMBER,       // Anumerical number 
    TOKEN_TYPE_STRING,       // A string with data between double qouts i.e  "Hello World!"
...
struct lex_process
{
    ...
    if we have ((50)) so the variable will be 2
...
struct compile_process
{
    ...
    int flags;
    struct pos pos;
    struct compile_porocess_input_file
    {
        FILE* fp;
        const char* abs_path;
    } cfile;
...
void* lex_process_get_private(struct lex_process* process);
